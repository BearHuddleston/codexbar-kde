...
class ModelTests(unittest.TestCase):
    def test_normalize_payload_extracts_provider_windows_and_omits_identity(self):
        ...
        self.assertEqual(
            [(w.key, w.label, w.used_percent) for w in providers[0].windows],
            [
    ...
    def test_normalize_payload_attaches_pace_notes_to_windows(self):
        ...
        self.assertEqual(
            notes["primary"], "30% reserve · expected 66% · lasts to reset"
        )
        self.assertEqual(
            notes["secondary"],
            "12% over pace · expected 40% · may run out before reset",
        )
    ...
    def test_reset_countdown_and_severity(self):
        ...
        self.assertEqual(
            format_reset_countdown("2026-07-04T01:02:50Z", now=now), "1h 3m"
        )
        self.assertEqual(
            format_reset_countdown("2026-07-07T03:01:46Z", now=now), "3d 3h"
        )
